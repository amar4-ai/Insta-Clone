...
export const addNewPost = async (req, res) => {
    try {
        ...
        //image upload
        const optimizadImageBuffer = await sharp(image.buffer)
            .resize({ width: 800, fit: 'inside' })
            .toFormat('jpeg', { quality: 80 })
            .toBuffer();

        // buffer to data uri
        const fileUri = `data:image/jpeg;base64,${optimizadImageBuffer.toString('base64')}`;
...
export const likePost = async (req, res) => {
    try {
        ...
        //like logic started from here
        await post.updateOne({ $addToSet: { likes: likeGarneUserId } });
        await post.save();
...
export const disLikePost = async (req, res) => {
    try {
        ...
        //like logic started from here
        await post.updateOne({ $pull: { likes: likeGarneUserId } });
        await post.save();
...
export const bookmarkPost = async (req, res) => {
    try {
        ...
        if (user.bookmarks.includes(post._id)) {
            ...
            await user.save();
            return res.status(200).json({
                type: 'unsaved', message: "Post removed from bookmark",
                success: true
            })
        } else {
            // do bookmark
            ...
            await user.save();
            return res.status(200).json({
                type: 'unsaved', message: "Post bookmarked",
                success: true
            })
        }
    } catch (error) {
